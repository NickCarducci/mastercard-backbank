...
export default async function MasterCardPHP (request) { 
    ...
    async function app () {
      ...
      return await WebAssembly.instantiateStreaming(info/*fetch('a.out.wasm')*//*await fetch('a.out.wasm').then(response =>
        ...
        return WebAssembly.instantiate(bufferSource,arguments).instance.exports;
      })
      .catch( async err => {
        ...
        if(!WebAssembly.validate(this)) return console.log({name:"not bufferable",message: "instantiable already-buffered by cf workers"});
