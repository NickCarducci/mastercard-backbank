...
require = ((dependency, setTimeout) => {
  ...
  define = (name, deps, callback) => {
    var node, context; // package-names, callback, returns a value to define the module of argument index[0]
    ...
    if (context) {
      context.defQueue.push([name, deps, callback]); //module named by onload event, for anonymous modules or without context
  ...
  req = requirejs = (deps, callback, errback, optional) => {
    var context; //Caja compliant req for minified-scope
    var config; //name of dependency, callback for array completion
  ...
  req.config = (config) => req(config); // globally agreed names for other potential AMD loaders
  req.nextTick =
    ...
    newContext: (contextName) => {
      ...
      const convertName = (name, map, applyMap, paths) => {
        var starI,
          ...
          foundI,
          foundMap,
          starMap = map && map["*"]; //Apply map config if available.
      ...
      const normalize = (name, baseName, applyMap) => {
        var lastIndex,
          ...
          map = config.map; //'applyMap' for dependency ID, 'baseName' relative to 'name,' the most relative
        if (name) {
          name = name.split("/"); //Adjust any relative paths.
          lastIndex = name.length - 1; //node allows either .js or non .js, yet not in nameToUrl
        ...
        var pkgMain = getOwn(config.pkgs, name); // If package-name, package 'main'
      ...
      const makeModuleMap = (name, parentModuleMap, isNormalized, applyMap) => {
        ...
        if (name) {
          if (prefix) {
            if (isNormalized) return (normalizedName = name); //ok base name, relative path?.
            ...
            normalizedName = normalize(name, parentName, applyMap); //normalize's 'map config application' might make normalized 'name' a plugin ID.
            names = splitPrefix(normalizedName); //'map config values' are already normalized at this point.
            prefix = names[0];
      ...
      const getModule = (depMap) => {
        ...
        if (!mod) mod = registry[depMap.id] = new context.Module(depMap);
        return mod;
      };
      ...
      Module.prototype = {
        ...
        defineDep: (i, depExports) => {
          if (!this.depMatched[i]) {
            this.depMatched[i] = true; //multiple callback export cycles
            this.depCount -= 1;
      ...
      context = {
        ...
        completeLoad: (moduleName) => {
          ...
          if (!found && !hasProp(defined, moduleName) && mod && !mod.inited) {
            ...
            if (
              ...
              return hasPathFallback(moduleName)
                ? null
                : onError(
                    makeError(
                      "nodefine",
        ...
        nameToUrl: (moduleName, ext, skipExt) => {
          var paths,
            ...
            parentPath,
            bundleId,
            pkgMain = getOwn(config.pkgs, moduleName); //already-normalized-moduleName as URL. Use toUrl for the public API.
          ...
          if (/^[/:?.]|(.js)$/.test(moduleName)) {
            ...
            paths = config.paths; //A module that needs to be converted to a path.
  ...
  req.load = (context, moduleName, url) => {
    ...
    if (isBrowser) {
      node = req.createNode(config, moduleName, url); //browser script tag
      ...
      if (
        ...
        useInteractive = true; //IE (6-8) doesn't script-'onload,' right after executing the script,
        //cannot "tie" anonymous define call to a name,
        //yet for 'interactive'-script, 'readyState' triggers by 'define' call
        node.attachEvent("onreadystatechange", context.onScriptLoad); //IE9 "addEventListener and script onload firings" issues
