...
require = ((dependency, setTimeout) => {
  ...
  const parseName = (nm, roots, conId) => nm &&
    (() => {nm = nm.split("/");
      ...
      if (suffjs) nm[l] = nm[l].replace(isjs, "");if (nm[0].charAt(0) === "." && roots)
  ...
  var Module = (map, unDE, configShim) => {
    ...
    Object.keys(obj).forEach((key) => (module.key = obj[key]));}; //module.exports; module.factory; module.depMaps = [], module.enabled, module.fetched
  // prettier-ignore
  var req = (REQUIREJS = (ds, cb, errback, optional) => {
    ...
    if (!exists(ds).string() === "[object Array]" && typeof ds !== "string") {
      ...
      if(exists(cb).string()==="[object Array]"){ds=cb;cb=errback;errback=optional;}else ds=[];}//ds is a CONFIG object Adjust args if there are dependencies
      ...
      callPlugin: () => {
        ...
        on(pluginMap, "defined", (plugin) => {
          ...
          load.error = (err) => {
            ...
            Object.keys(registry).forEach((x,i)=>registry[x].map.id.indexOf(id+"_unnormalized")===0&&clrRegstr(registry[x].map.id));
            onError(err);
