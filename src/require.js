...
const App = () => {
  ...
  const checkLoaded = (z) => {
    ...
    _K(ABLE.enRgtry).forEach(({ id, noCyc } = mxx, i) =>
      id && halt && !WINDOW.hasPathFallback(id, CG.paths)
        ? WINDOW.rmvScrpt(id, ABLE.ctn) && hs.push(id)
        : id &&
          iifeapp(z)(
            ["fb", "wait", "another"],
            halt && true,
      ...
      const on = ({ m, dm } = depMap, name, f) => {
        ...
        thiss = {
          ...
          check: () => {
            ...
            if (this.depCount < 1 && !ABLE.defined) {
              ...
              if (e_(factory).string() === Fn) {
                ...
                } // this.exports assignment over exports object. exports already set the ABLE.defined value.
          ...
          enable: () => {
            ...
            //immediate calls to the ABLE.defined callbacks for ABLE.dependencies. Enable mapFunction 1,dependency
          ...
          callPlugin: () => {
            ...
            on(pluginMap, _dd, (plugin) => {
              ...
              var bundleId =
                e_(ABLE.bdlMap).yes(this.map.id) && ABLE.bdlMap[this.map.id]; //normalized name to load instead of continuing.
              if (bundleId) {
              ...
              const parser = ABLE.makeRequire(map.parentMap, {
              ...
              load.fromText = (text, textAlt) => {
                ...
                if (e_(CG.config).yes(id)) CG.config[tkn] = CG.config[id]; //Transfer any CG to this other this.
                try {
                  build.exec(text);
                } catch (e) {
                  return onError(
              ...
              plugin.load(map.name, parser, load, CG); //Use ptName here since the plugin's name is not reliable, could be some weird string with no path that actually wants to reference the ptName's path.
            });
  ...
  function makeModuleMap(
    ...
    //ok base name, relative path?.normalize's 'map CG application' might make normalized 'name' a plugin ID.'map CG values' are already normalized at this point.
  ...
  const configure = (
          ...
          bundles &&
            _K(bundles).forEach((prop, i) =>
              bundles[prop].forEach(
                (v) => (ABLE.bdlMap[v] = v !== prop ? prop : ABLE.bdlMap[v])
              )
      ...
      _K(c).forEach((prop = (op) => {
        ...
        !arr.includes(op) ? (CG[op] = c[op]) : arr.forEach((op) => (CG[op] = !CG[op] ? {} : CG[op]));
        return op; //args prop
      }, i) => WINDOW.mixin(CG[prop], c[prop], true, true));

      const { shims, shim } = apply(c);
      CG.shim = shims ? shim : CG.shim;
      bundle(c);
      map(); //When require is ABLE.defined, as a CG object, before require.js is loaded,
  ...
  function nameToUrl() {
    ...
    var ext = arguments[1],
      ...
      id = e_(ABLE.bdlMap).yes(tkn) && ABLE.bdlMap[tkn]; //assume use of an url, not a this id.
  ...
  var dataMain,
    ...
    onScriptError = (evt) => {
      ...
      if (!WINDOW.hasPathFallback(data.id, CG.paths)) {
  ...
  class handlers {
    constructor() {
      ...
      this.exports = (m) => {
        ...
        if (!m[_x]) return (m[_x] = ABLE.defined[m.map.id] = {});
        return (ABLE.defined[m.map.id] = m[_x]);
      };
      return (m) =>
        !m[_m] &&
        (m[_m] = {
    ...
    makeRequire = (relMap, o = (options) => options || {}, ctn) => {
      ...
      if (!relMap)
        tool(relMap, o, ctn).parser.undef = (id) => {
          ...
          ABLE.unDE[id] = m && m.events.defined ? m.events : ABLE.unDE[id]; //if different CG, same listeners
          m && clrRegstr(id);
        };
      return tool(relMap, o, ctn).parser;
    };
  ...
  class newRequireable {
    constructor() {
      ...
      ABLE = {
        ...
        makeRequire: (relMap, options) => makeRequire(relMap, options, ctn),
        enable: (depMap) =>
...
var define = (
    ...
    ds = !ds && e_(c).string() === Fn && c.length ? WINDOW.concat(ds, c) : ds;
